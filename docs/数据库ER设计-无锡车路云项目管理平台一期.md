...
用途：统一保存各分项 sheet 中的硬件、软件、服务、施工和资源明细。
...
- `original_factory_amount`
- `gross_profit_amount`
- `related_flow_line_id`
- `related_flow_edge_id`
- `front_contract_id`
- `back_contract_id`
...
### 3.24 合同文件解析表 `contract_document_parse`

用途：记录前项天安销售、后项天安采购盖章合同 PDF 的扫描、OCR、解析和人工复核结果。
...
- `ocr_status`
- `parsed_contract_no`
- `parsed_contract_name`
- `parsed_party_a`
- `parsed_party_b`
- `parsed_amount`
- `parsed_sign_date`
- `parsed_payment_terms`
- `parsed_delivery_terms`
- `manual_review_status`
- `parse_message`

### 3.25 合同付款节点表 `contract_payment_node`

用途：按合同配置签署、到货、入库、安装、验收、质保等付款节点。
...
- `actual_amount`
- `actual_date`
- `upstream_receipt_required`
- `upstream_payment_node_id`
- `payment_status`

### 3.26 设备级履约事件表 `contract_item_fulfillment_event`
...
### 3.27 设备级资金流水表 `contract_item_cash_flow`

用途：将前向回款、后向付款、质保金、回流业主车联网集团等资金流细化到设备或合同明细级。
...
- `actual_date`
- `trigger_event_id`
- `upstream_cash_flow_id`
- `owner_backflow_required`
- `owner_backflow_status`
- `cash_flow_status`
